...
<div class="col-lg-12">
    <div class="card">
        ...
        <form action="{{ route('edit-type',[$idd->id]) }}" method="post" id="formUpdateAdmin" enctype="multipart/form-data" class="form-horizontal edit_admin">
            @csrf
            @foreach ($getId as $aa)
...
<input name="img_edit" type="file" accept="image/*" onchange="document.getElementById('output').src = window.URL.createObjectURL(this.files[0])">
        ...
        <div class="card-footer">
            <div class="row form-group">
                <input type="submit" value="Sửa thể loại" class="btn btn-primary">
            </div>
        </div>  
        </form>
    </div>
